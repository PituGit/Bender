...
Game::Game(unsigned int a, unsigned int b) 
    : resolucioX(a), resolucioY(b)
{
  GameStatus.Exit = false;
  GameStatus.Minimized = 0;
  GameStatus.CurrentFrameNumber = 0;
}
...
void Game::VideoUpdate()
{
  // Incrementamos el n?mero de frame actual
  GameStatus.CurrentFrameNumber++ ;
...
void Game::Video_Init(int new_resx, int new_resy, bool FullScreen)
{
...
void Game::ProcessEventSystem()
{
  ...
  {
    // Application events        
    if(e.type == SDL_QUIT)   GameStatus.Exit = true;
       
    // Window events        
    else if(e.type == SDL_WINDOWEVENT)
    {
      if(e.window.event == SDL_WINDOWEVENT_MINIMIZED)   GameStatus.Minimized = true;            
      else if(e.window.event == SDL_WINDOWEVENT_RESTORED)    GameStatus.Minimized = false;
    }
...
void Game::Video_Flip()
{
  // Pide al sistema operativo que actualice el contenido visible de la ventana
  SDL_RenderPresent(g_Video.renderer) ;
}

void Game::InitializeHardware(int new_resolution_x, int new_resolution_y, int fps_maximos, bool FullScreen)
{
  ...
  Keyboard_Init();
  Mouse_Init();
  Video_Init(new_resolution_x, new_resolution_y, FullScreen);
}
